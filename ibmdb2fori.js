...
module.exports = function(RED) {    
    ...
    function ibmdb2foriNode(n) {
        ...
        function doConnect(conncb) {
            ...
            //Beta version - query attributes [{"id": 2, "value": 3 },{"id": 12, "value": 2 } ]
            node.debug("DB2 Connection Attributes: "+node.attributes); 
            if (node.attributes != null && ''+node.attributes!="" && node.attributes != "[]")
              {
                try{
                    attrArray=JSON.parse(node.attributes)
                    for(var i = 0; i < attrArray.length; i++)
                        {
                          this.dbconn.setConnAttr(parseInt(attrArray[i].id), parseInt(attrArray[i].value))  ; 
                        }
                    
                }
                catch(e){
                    node.error("Error setting connection attributes: "+node.attributes+ " . Please check ids and values.");
                }
